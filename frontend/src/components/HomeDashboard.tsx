...
const FarmerDashboard = ({ onNavigate }: { onNavigate: any }) => {
  ...
  const verificationLabel  = verificationStatus === "verified" ? t("profile.verified")
                           : verificationStatus === "auto_verified" ? t("profile.verified")
  ...
  useEffect(() => {
    const fetchTrust = async () => {
      if (!userId) return;
      try {
        const res = await apiClient.get<APIResponse<any>>(`/trust-score/${userId}`);
        if (res.data.status === "success") {
          setTrustScore(res.data.data.score);
        }
      } catch (e) { console.error("Could not fetch trust score", e); }
    };
    fetchTrust();
  }, [userId]);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-[#408447] text-white p-6 shadow-sm border border-[#13311c]/20">
         <h2 className="font-mukta font-bold text-2xl">{t("dashboard.welcomeBack")}, {userName}!</h2>
         <div className="mt-4 flex flex-wrap gap-4">
            <div className="bg-black/10 px-4 py-2 flex items-center gap-2">
               <Store className="w-4 h-4 text-[#d4cb7e]" />
               <span className="font-bold text-sm">{t("nav.mandiPrices")}</span>
            </div>
            <div className="bg-black/10 px-4 py-2 flex items-center gap-2">
               <Activity className="w-4 h-4 text-green-300" />
               <span className="font-bold text-sm">{t("profile.verificationStatus")}: {verificationLabel}</span>
            </div>
            {trustScore !== null && (
              <div className="bg-black/10 px-4 py-2 flex items-center gap-2" title="Generated by AI based on profile data">
                 <ShieldCheck className="w-4 h-4 text-amber-300" />
                 <span className="font-bold text-sm">{t("dashboard.trustScore")}: {trustScore}/100</span>
              </div>
            )}
         </div>
      </div>
      ...
      <h3 className="font-bold text-xs uppercase text-gray-500 tracking-wider">{t("dashboard.activeModules")}</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
         <button onClick={() => onNavigate("mandi")} className="bg-white border border-[#e5e3d7] p-6 hover:border-[#408447] flex flex-col justify-center gap-3 transition-colors shadow-sm">
            ...
            <span className="font-bold text-[13px] text-center leading-tight">{t("nav.mandiPrices")}</span>
         </button>
         ...
         <button onClick={() => onNavigate("fraud")} className="bg-white border border-[#e5e3d7] p-6 hover:border-[#c82b28] flex flex-col justify-center gap-3 transition-colors shadow-sm">
            ...
            <span className="font-bold text-[13px] text-center leading-tight">{t("nav.inputVerification")}</span>
         </button>
         ...
         <button onClick={() => onNavigate("loan")} className="bg-white border border-[#e5e3d7] p-6 hover:border-[#3174a1] flex flex-col justify-center gap-3 transition-colors shadow-sm">
            ...
            <span className="font-bold text-[13px] text-center leading-tight">{t("nav.loanDecoder")}</span>
         </button>
         ...
         <button onClick={() => onNavigate("legal")} className="bg-white border border-[#e5e3d7] p-6 hover:border-[#e18b2c] flex flex-col justify-center gap-3 transition-colors shadow-sm">
            ...
            <span className="font-bold text-[13px] text-center leading-tight">{t("nav.legalAid")}</span>
         </button>
      </div>
